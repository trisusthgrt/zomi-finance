...
export default function DashboardV2() {
    ...
    const {
        infoTokens: infoTokensArbitrum
    } = useInfoTokens(null, ARBITRUM, active, undefined, undefined);
    const {
        infoTokens: infoTokensAvax
    } = useInfoTokens(null, MAINNET, active, undefined, undefined);
    ...
    let {
        total: totalGmxInLiquidity
    } = useTotalGmxInLiquidity(chainId, active);
    ...
    const CustomTooltip = ({
        ...
        if (active && payload && payload.length) {
            return ( <
                ...
                <
                div className = "stats-label-color"
                style = {
                    {
    ...
    renderContent = {
        () => ( <
            span > {
                t `Assets Under Management: $ZOMI staked (All chains) + $ZLP pool (${chainName})`
            } < /span>
    ...
    renderContent = {
            () => < span > {
                t `Total value of tokens in $ZLP pool (${chainName})`
            } < /span>} /
            >
            <
            /div> <
            ...
            renderContent = {
                () => ( <
                    ...
                    avaxValue = {
                        totalVolumeDelta ? formatNumber(totalVolumeDelta, {
                            currency: true,
                            compact: false
                        }) : `$0`
                    }
            ...
            /div> {
                feesSummary.lastUpdatedAt ? ( <
                    ...
                    div >
                    <
                    TooltipComponent position = "right-bottom"
                    className = "nowrap"
                    handle = {
            ...
            handle = {
                totalFees ? `${formatNumber(totalFees, { currency: true, compact: false })}` : `$0`
            }
            renderContent = {
                () => ( <
                    StatsTooltip title = {
    ...
    width = "40px" / >
        ...
        /div> <
        div > {
            formatAmount(totalGmxSupply, GMX_DECIMALS, 0, true)
        }
    $ZOMI < /div> <
        /div> <
        div className = "App-card-row" >
    ...
    width = "40px" / > {
        ...
        /div> <
        div > {
            formatAmount(glpSupply, GLP_DECIMALS, 0, true)
        }
    $ZLP < /div> <
        /div> <
        div className = "App-card-row" >
            ...
            glpPool.length > 0 && ( <
                ...
                } >
                {
                    glpPool.map((entry, index) => ( <
                        ...
                        fill = {
                            GLP_POOL_COLORS[entry.name]
                        }
                        style = {
                            {
                                filter: glpActiveIndex === index ?
                                ...
                                renderContent = {
                                    () => {
                                        return ( <
                                            ...
                                            value = {
                                                `${formatKeyAmount(tokenInfo, "managedAmount", token.decimals, 0, true)} ${
                                      token.symbol
                                    }`
                                            }
                                            showDollar = {
                                            ...
                                            value = {
                                                `${formatKeyAmount(tokenInfo, "bufferAmount", token.decimals, 0, true)} ${
                                      token.symbol
                                    }`
                                            }
                                            showDollar = {
                                            ...
                                            /> <
                                            StatsTooltipRow label = {
                                                t `Max ${tokenInfo.symbol} Capacity`
                                            }
                                            value = {
