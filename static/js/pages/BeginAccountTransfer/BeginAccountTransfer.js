...
export default function BeginAccountTransfer(props) {
    ...
    const {
        data: gmxVesterBalance
    } = useSWR([active, chainId, gmxVesterAddress, "balanceOf", account], {
        fetcher: contractFetcher(library, Token),
    });

    const {
        data: glpVesterBalance
    } = useSWR([active, chainId, glpVesterAddress, "balanceOf", account], {
        fetcher: contractFetcher(library, Token),
    });

    const stakedGmxTrackerAddress = getContract(chainId, "StakedGmxTracker");
    const {
        data: cumulativeGmxRewards
    } = useSWR(
    ...
    const {
        data: transferredCumulativeGmxRewards
    } = useSWR(
    ...
    const {
        data: transferredCumulativeGlpRewards
    } = useSWR(
    ...
    const {
        data: pendingReceiver
    } = useSWR([active, chainId, rewardRouterAddress, "pendingReceivers", account], {
        fetcher: contractFetcher(library, RewardRouter),
    });

    const {
        data: gmxAllowance
    } = useSWR([active, chainId, gmxAddress, "allowance", account, stakedGmxTrackerAddress], {
        fetcher: contractFetcher(library, Token),
    });
    ...
    return ( <
        ...
        <
        Modal isVisible = {
            isTransferSubmittedModalVisible
        }
        setIsVisible = {
            setIsTransferSubmittedModalVisible
        }
        label = {
        ...
        for full account transfers. <
        br / >
        This will transfer all your $ZOMI, esZOMI, $ZLP and Multiplier Points to your new account. <
        br / >
        Transfers are only supported
        if the receiving account has not staked $ZOMI or $ZLP tokens before. <
        br / >
        ...
        /div> {
            hasPendingReceiver && ( <
                ...
                You have a < Link to = {
                    pendingTransferLink
                } > pending transfer < /Link> to {pendingReceiver}. <
                /Trans> <
                /div>
            )
        } <
