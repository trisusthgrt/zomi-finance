...
export function useAllOrders(chainId, library) {
    ...
    useEffect(() => {
        getGmxGraphClient(chainId).query({
            query
        }).then(setRes);
    }, [setRes, query, chainId]);
...
export function useGmxPrice(chainId, libraries, active) {
    ...
    const {
        data: gmxPriceFromEthereum,
        mutate: mutateFromEthereum
    } = useGmxPriceFromEthereum();
...
function useGmxPriceFromEthereum() {
    ...
    const {
        data,
        mutate: updateReserves
    } = useSWR(["TraderJoeGmxAvaxReserves", MAINNET, poolAddress, "getReserves"], {
        fetcher: contractFetcher(undefined, UniswapV2),
    });
    const {
        _reserve0: gmxReserve,
...
export function useTotalGmxSupply() {
    const {
        data: gmxSupply
    } = useSWR([`StakeV2:totalSupply:${MAINNET}`, MAINNET, getContract(MAINNET, "GMX"), "totalSupply"], {
        fetcher: contractFetcher(undefined, Token),
    });
...
export function useTotalGmxStaked() {
    ...
    const {
        data: stakedGmxSupplyEth,
        mutate: updateStakedGmxSupplyEth
    } = useSWR(
        [
...
export function useTotalGmxInLiquidity() {
    ...
    const {
        data: gmxInLiquidityOnEth,
        mutate: mutateGMXInLiquidityOnEth
    } = useSWR(
