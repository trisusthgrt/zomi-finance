...
function AffiliatesStats({
    ...
    const finalAffiliatesTotalStats = useMemo(
        ...
        recentlyAddedCodes.filter(isRecentReferralCodeNotExpired).reduce((acc, cv) => {
            ...
            return acc;
        }, referrerTotalStats), [allReferralCodes, referrerTotalStats, recentlyAddedCodes]
    );
    ...
    return ( <
        ...
        <
        InfoCard label = {
            t `Total Traders Referred`
        }
        tooltipText = {
            t `Amount of traders you referred.`
        }
        data = {
        ...
        /> <
        InfoCard label = {
            t `Total Trading Volume`
        }
        tooltipText = {
            t `Volume traded by your referred traders.`
        }
        data = {
        ...
        tooltipText = {
            t `Rebates earned by this account as an affiliate.`
        }
        data = {
        ...
        <
        AffiliateCodeForm handleCreateReferralCode = {
            handleCreateReferralCode
        }
        recentlyAddedCodes = {
            recentlyAddedCodes
        ...
        tbody > {
            finalAffiliatesTotalStats.map((stat, index) => {
                ...
                return ( <
                    ...
                    /div> <
                    a href = {
                        getTwitterShareUrl(stat.referralCode)
                    }
                    target = "_blank"
                    ...
                    td data - label = "Traders Referred" > {
        ...
        /div> {
            rebateDistributions ? .length > 0 ? ( <
                ...
                <
                Card title = {
                    t `Rewards Distribution History`
                }
                tooltipText = {
                    t `Rewards are airdropped weekly.`
                } >
                <
                div className = "table-wrapper" >
                <
                ...
                tbody > {
                    rebateDistributions.map((rebate, index) => {
                        ...
                        return ( <
                            ...
                            td className = "table-head"
                            data - label = "Amount" > {
                                formatAmount(rebate.amount, tokenInfo.decimals, 6, true)
                            } {
                                tokenInfo.symbol
                            } <
                            /td> <
                            ...
                            <
                            ExternalLink href = {
                                explorerURL + `tx/${rebate.transactionHash}`
                            } > {
                ...
                EmptyMessage tooltipText = {
                    t `Rebates are airdropped weekly.`
                }
                message = {
                    t `No rebates distribution history yet.`
                }
                />
            )
        } <
