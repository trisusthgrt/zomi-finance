...
export default function GlpSwap(props) {
    ...
    const {
        data: aums
    } = useSWR([`GlpSwap:getAums:${active}`, chainId, glpManagerAddress, "getAums"], {
    ...
    const getError = () => {
        ...
        if (isBuying) {
            const swapTokenInfo = getTokenInfo(infoTokens, swapTokenAddress);
            if (!savedShouldDisableValidationForTesting &&
                swapTokenInfo &&
    ...
    return ( <
            ...
            /div> <
            div className = "value" > {
                formatAmount(glpBalance, GLP_DECIMALS, 4, true)
            }
            $ZLP($ {
            ...
            div className = "App-card-divider" / >
            <
            div className = "App-card-content" > {!isBuying && ( <
                    div className = "App-card-row" >
                    <
                    div className = "label" >
                    <
                    ...
                    <
                    Tooltip handle = {
                        `${formatAmount(reservedAmount, 18, 4, true)} $ZLP ($${formatAmount(
                      reserveAmountUsd,
                      USD_DECIMALS,
                      2,
                      true
            ...
            renderContent = {
                () => {
                    return ( <
                        ...
                        <
                        StatsTooltipRow label = {
                            t `${nativeTokenSymbol} (${wrappedTokenSymbol}) APR`
                        }
                        value = {
                            `${formatAmount(feeGlpTrackerApr, 2, 2, false)}%`
                        }
                        showDollar = {
                            false
                        }
            ...
            <
            Trans > {
                formatAmount(glpSupply, GLP_DECIMALS, 4, true)
            }
            $ZLP($ {
            ...
            > {
                isBuying && ( <
                    ...
                    tokenBalance = {
                        `${formatAmount(swapTokenBalance, swapToken.decimals, 4, true)}`
                    }
                    inputValue = {
                        swapValue
                    }
...

            {
                !isBuying && ( <
                    ...
                    tokenBalance = {
                        `${formatAmount(maxSellAmount, GLP_DECIMALS, 4, true)}`
                    }
                    inputValue = {
                        glpValue
                    }
                    ...
                    showMaxButton = {
                        glpValue !== formatAmountFree(maxSellAmount, GLP_DECIMALS, GLP_DECIMALS)
                    }
                    onClickTopRightLabel = {
                        fillMaxAmount
                    }
                ...
                isBuying && ( <
                    ...
                    renderContent = {
                        () => {
                            ...
                            return ( <
                                div className = "text-white" > {
                                    feeBasisPoints > 50 && < Trans > To reduce fees,
                                    select a different asset to pay with. < /Trans>} <
                                    Trans > Check the "Save on Fees"
                                    section below to get the lowest fee percentages. < /Trans> <
                                        /div>
                                );
                            }
                        }
                            ...
                            renderContent = {
                                () => {
                                    ...
                                    return ( <
                                        div className = "text-white" > {
                                            feeBasisPoints > 50 && < Trans > To reduce fees,
                                            select a different asset to receive. < /Trans>} <
                                            Trans > Check the "Save on Fees"
                                            section below to get the lowest fee percentages. < /Trans> <
                                                /div>
                                        );
                    ...
                    disabled = {!isPrimaryEnabled()
                        ...
                        /div> {
                            ...
                            !isBuying && ( <
                                ...
                                <
                                Trans >
                                Fees may vary depending on which asset you sell $ZLP
                                for. < br / >
                        ...
                        /div> <
                        div className = "GlpSwap-token-list" > { /* <div className="GlpSwap-token-list-content"> */ } <
                    ...
                    th > < /th> <
                      ...
                      tokenFeeBps !== undefined && tokenFeeBps.toString().length > 0 ? "%" : ""
                                ...
                                return ( <
                                    ...
                                    <
                                    button className = {
                                        cx("App-button-option action-btn", isBuying ? "buying" : "selling")
                                    }
                                    onClick = {
                                        () => selectToken(token)
                                    } >
