    ...
    return ( <
        ...
        renderContent = {
            () => ( <
                ...
                value = {
                    `-$${formatAmount(liquidationData.loss, USD_DECIMALS, 2, true)}`
                }
                /> {
                    liquidationData.type === "full" && ( <
                        StatsTooltipRow label = {
                        ...
                        showDollar value = {
                            formatAmount(LIQUIDATION_FEE, 30, 2, true)
                        }
                        />
                    )
                } <
            ...
            if (tradeData.action === "CancelIncreasePosition") {
                ...
                return ( <
                    ...
                    Could not increase {
                        ...
                        `+${formatAmount(bigNumberify(params.sizeDelta), USD_DECIMALS, 2, true)}`
                    }
                    USD, Acceptable Price: & nbsp; {
                        params.isLong ? "<" : ">"
                    } & nbsp; <
                    /Trans> <
                    Tooltip position = "left-top"
                ...
                if (isLiquidation && liquidationData) {
                    return ( <
                        ...
                        USD, {
                            ...
                            formatAmount(params.price, USD_DECIMALS, 2, true)
                        }
                        USD <
                        />
                    );
                ...
                if (liquidationData) {
                    return ( <
                        Trans > {
                            ...
                            formatAmount(params.size, USD_DECIMALS, 2, true)
                        }
                        USD, & nbsp; {
                            indexToken.symbol
                        }
                        Price: $ {
                            formatAmount(params.markPrice, USD_DECIMALS, 2, true)
                        }
                        USD <
                        /Trans>
                    );
        ...
        div className = "TradeHistory" > {
            ...
            tradesWithMessages.length > 0 &&
                tradesWithMessages.map((trade, index) => {
                        ...
                        return ( <
                            div className = "TradeHistory-row App-box App-box-border"
                            key = {
                                index
                            } >
                            <
